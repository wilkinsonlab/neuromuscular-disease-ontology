...
1. Add an import statement to your `src/ontology/nmdo-edit.owl` file. We suggest to do this using a text editor, by simply copying an existing import declaration and renaming it to the new ontology import, for example as follows:
    ```
    ...
    Ontology(<http://purl.obolibrary.org/obo/nmdo.owl>
    Import(<http://purl.obolibrary.org/obo/nmdo/imports/ro_import.owl>)
...
in the catalog file to redirect the URL `http://purl.obolibrary.org/obo/nmdo/imports/go_import.owl`
...
2. remove the id from your `src/ontology/nmdo-odk.yaml`, eg. `- id: go` from the list of `products` in the `import_group`.
...
The following change to your repo config (`src/ontology/nmdo-odk.yaml`) will switch the go import from an SLME module to a simple ROBOT filter module:
...
If you wish to customise your import entirely, you can specify your own ROBOT command to do so. To do that, add the following to your repo config (`src/ontology/nmdo-odk.yaml`):
...
Now add a new goal in your custom Makefile (`src/ontology/nmdo.Makefile`, _not_ `src/ontology/Makefile`).
...
1. Providing a `components/mycomp.owl:` make target in `src/ontology/nmdo.Makefile`
...
1. Providing an additional attribute for the component in `src/ontology/nmdo-odk.yaml`, `source`,
